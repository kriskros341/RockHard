...
export const deepComparison = (o1: any, o2: any, excuseTypeDifferences: boolean = false): boolean => {
  ...
  if(excuseTypeDifferences) {
    if(typeof o1 != 'object' && String(o1) === String(o2)) {
      return true
    }
  } else {
    if(o1 === o2) {
  ...
  for(let i of keysOf1) {
    if(!keysOf2.includes(i)) {
      return false
    }
    if(!excuseTypeDifferences && o1[i] !== o2[i] && typeof o1[i] != 'object') {
      return false
    }
    if(o1[i] != o2[i] && typeof o1[i] == 'object' && o1[i] != null) {
      subObjects = [
        ...subObjects, 
        deepComparison(o1[i], o2[i], excuseTypeDifferences)
      ]
    }
  }
  ...
  return (
      React.Children.map(children, (Item, index) => 
        fn(index) &&
          React.createElement(
            React.Fragment, 
            {key: `renderedConditionally__${index}`}, 
            Item
          )
    )
  )
